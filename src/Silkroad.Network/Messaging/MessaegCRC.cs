...
///     This was made to ensure that the <see cref="Message" /> wasn't interrupted by a third-party.
/// </summary>
// ReSharper disable once InconsistentNaming
internal sealed class MessageCRC {
    /// <summary>
    ///     The internal CRC lookup table seed.
    /// </summary>
    private readonly uint _seed;

    /// <summary>
    ///     Initializes a CRC with a specific seed.
    /// </summary>
    /// <param name="seed">The CRC lookup table seed.</param>
    public MessageCRC(uint seed) {
        this._seed = seed << 8;
    }

    /// <summary>
    ///     Computes the CRC of a specific buffer.
    /// </summary>
    /// <param name="buffer">The buffer to compute it's CRC.</param>
    /// <returns>The buffer's CRC checksum.</returns>
    public byte Compute(in Span<byte> buffer) {
        var checksum = uint.MaxValue;

        foreach (var b in buffer) checksum = (checksum >> 8) ^ Table[this._seed + ((b ^ checksum) & byte.MaxValue)];

        return (byte)(((checksum >> 24) & byte.MaxValue) + ((checksum >> 8) & byte.MaxValue) +
                      ((checksum >> 16) & byte.MaxValue) + (checksum & byte.MaxValue));
    }

    #region CRC lookup table generation

    private static readonly uint[] Table = GenerateTable();

    private static uint[] GenerateTable() {
        var baseTable = new uint[] {
    ...
    #endregion CRC lookup table generation
}