...
internal class ClientMessageProtocol : MessageProtocol {
    ...
    protected override void Validate(Message msg) {
        ...
        if (msg.Sequence != 0) throw new InvalidMessageException(InvalidMessageReason.InvalidSequence);

        if (msg.CRC != 0) throw new InvalidMessageException(InvalidMessageReason.InvalidCRC);
    }
