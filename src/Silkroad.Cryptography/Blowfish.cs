...
namespace Silkroad.Cryptography {
    ...
    public class Blowfish {
        private static readonly uint[] PArray = {
        ...
        /// <summary>
        ///     Encrypts the input, its length doesn't have to be dividable by 8, as it would be
        ///     rounded and copied before encryption, returning the final output dividable by 8.
        /// </summary>
        /// <param name="input">The buffer to be encrypted.</param>
        /// <returns>The encrypted buffer, it's size is ensured to be dividable by 8.</returns>
        ...
        /// <summary>
        ///     Decrypts the input, its length has to be dividable by 8.
        /// </summary>
        /// <param name="input">The buffer to be decrypted.</param>
        /// <returns>
        ///     The decrypted buffer, it's size is ensured to be dividable by 8 and it might have zeros (0) padding at the end
        ///     of the original encrypted buffer was not dividable by 8.
        /// </returns>
        /// <exception cref="ArgumentOutOfRangeException">input length is not dividable by 8.</exception>
        public Span<byte> Decrypt(ReadOnlySpan<byte> input) {
            if (input.Length % 8 != 0 || input.Length == 0) {
                throw new ArgumentOutOfRangeException(nameof(input.Length));
            }

            var output = new byte[input.Length].AsSpan();
            input.CopyTo(output);
        ...
        /// <summary>
        ///     Decrypts the input in place, its length has to be dividable by 8.
        /// </summary>
        /// <param name="input">The buffer to be decrypted.</param>
        /// <returns>
        ///     The decrypted buffer, it's size is ensured to be dividable by 8 and it might have zeros (0) padding at the end
        ...
        /// </returns>
        /// <exception cref="ArgumentOutOfRangeException">input length is not dividable by 8.</exception>
        public void Decrypt(Span<byte> input) {
            if (input.Length % 8 != 0 || input.Length == 0) {
                throw new ArgumentOutOfRangeException(nameof(input.Length));
            }
        ...
        private uint F(uint x) {
            return ((this._sBoxes[0, (x >> 24) & byte.MaxValue] + this._sBoxes[1, (x >> 16) & byte.MaxValue])
                    ^ this._sBoxes[2, (x >> 8) & byte.MaxValue]) + this._sBoxes[3, (x >> 0) & byte.MaxValue];
        }
        ...
        private void Decrypt(ref uint left, ref uint right) {
            for (var i = 16; i > 0; i -= 2) {
                left ^= this._pArray[i + 1];
                right ^= this.F(left);
                right ^= this._pArray[i];
                left ^= this.F(right);
            }
        ...
        /// <summary>
        ///     Calculates the required buffer size that can be processed, as the Blowfish IV is 8.
        ///     Which means that the buffer size should be dividable by 8. This method ensure that.
        /// </summary>
        /// <param name="length">The input buffer length.</param>
        /// <returns>The length rounded up to the bigger number dividable by 8.</returns>
